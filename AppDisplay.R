      ...
      tabItem(
        ...
        fluidRow(
          box(
            ...
            collapsible = TRUE,
            collapsed = TRUE,
            verbatimTextOutput("spc_debug_info")
          )
        ),
        fluidRow(
          box(
...
server <- function(input, output, session) {
  ...
  # Create reactive values for debug info
  debug_values <- reactiveValues(info = NULL)
  ...
  spc_stats <- reactive({
    ...
    debug_info <- list()
    debug_info$step <- "1. Data Loading"
    debug_info$data_is_null <- is.null(data)
    debug_info$data_rows <- if (is.null(data)) 0 else nrow(data)
    debug_info$selected_year <- input$spc_year
    
    if (is.null(data) || nrow(data) == 0) {
      debug_info$error <- "Data is NULL or has 0 rows"
      debug_values$info <- debug_info
      return(NULL)
    }
    ...
    debug_info$step <- "2. Column Check"
    debug_info$available_cols <- names(data)
    debug_info$required_cols <- required_cols
    debug_info$missing_cols <- missing_cols
    
    if (length(missing_cols) > 0) {
      debug_info$error <- paste("Missing columns:", paste(missing_cols, collapse = ", "))
      debug_values$info <- debug_info
      return(NULL)
    }
    ...
    tryCatch({
      ...
      debug_info$step <- "2.5. Pre-Filter Analysis"
      debug_info$is_weekend_type <- class(data$Is_Weekend)
      debug_info$is_weekend_unique <- unique(data$Is_Weekend)
      debug_info$is_weekend_summary <- table(data$Is_Weekend, useNA = "always")
      debug_info$month_type <- class(data$Month)
      debug_info$month_unique <- sort(unique(data$Month))
      debug_info$month_summary <- table(data$Month, useNA = "always")
      
      # Check if Is_Weekend needs conversion (might be character "TRUE"/"FALSE")
      if (is.character(data$Is_Weekend)) {
        data$Is_Weekend <- as.logical(data$Is_Weekend)
        debug_info$is_weekend_converted <- TRUE
      } else {
        debug_info$is_weekend_converted <- FALSE
      }
      ...
      debug_info$step <- "3. After Filtering"
      debug_info$rows_after_filter <- nrow(data_filtered)
      debug_info$unique_stations <- if (nrow(data_filtered) > 0) length(unique(data_filtered$Start_Station_Code)) else 0
      if ("Date" %in% names(data_filtered) && nrow(data_filtered) > 0) {
        debug_info$unique_dates <- length(unique(data_filtered$Date))
      } else {
        debug_info$unique_dates <- if (nrow(data_filtered) == 0) 0 else "Date column not found"
      }
      
      if (nrow(data_filtered) == 0) {
        debug_info$error <- "No rows after filtering (weekdays, non-winter)"
        debug_info$weekday_count <- sum(!data$Is_Weekend, na.rm = TRUE)
        debug_info$non_winter_count <- sum(!data$Month %in% c(12, 1, 2), na.rm = TRUE)
        debug_info$weekday_non_winter_count <- sum(!data$Is_Weekend & !data$Month %in% c(12, 1, 2), na.rm = TRUE)
        debug_values$info <- debug_info
        return(NULL)
      }
      ...
      debug_info$step <- "4. After Grouping"
      debug_info$rows_after_grouping <- nrow(station_stats)
      if (nrow(station_stats) > 0) {
        debug_info$min_n_days <- min(station_stats$n_days, na.rm = TRUE)
        debug_info$max_n_days <- max(station_stats$n_days, na.rm = TRUE)
        debug_info$stations_with_30plus_days <- sum(station_stats$n_days >= 30, na.rm = TRUE)
      } else {
        debug_info$min_n_days <- 0
        debug_info$max_n_days <- 0
        debug_info$stations_with_30plus_days <- 0
      }
      
      station_stats <- station_stats %>%
        dplyr::filter(n_days >= 30, !is.na(mean_rides), !is.na(sd_rides), mean_rides > 0)
      
      debug_info$step <- "5. After Filtering (n_days >= 30)"
      debug_info$rows_after_filter30 <- nrow(station_stats)
      
      if (nrow(station_stats) == 0) {
        debug_info$error <- "No stations with >= 30 days of data"
        debug_values$info <- debug_info
        return(NULL)
      }
      ...
      debug_info$step <- "6. Final Result"
      debug_info$final_rows <- nrow(station_stats)
      ...
      debug_info$error <- paste("Error:", e$message)
  ...
  # Debug output
  output$spc_debug_info <- renderText({
    if (is.null(debug_values$info)) {
      return("Debug info not available yet. Select a year to see debug information.")
    }
    
    info <- debug_values$info
    
    output_text <- paste0(
      "=== SPC Stats Debug Information ===\n\n",
      "Step: ", info$step, "\n",
      "Selected Year: ", if (is.null(info$selected_year)) "NULL" else info$selected_year, "\n",
      "Data is NULL: ", info$data_is_null, "\n",
      "Data Rows: ", info$data_rows, "\n"
    )
    
    if (!is.null(info$available_cols)) {
      output_text <- paste0(output_text, "\nAvailable Columns: ", paste(info$available_cols, collapse = ", "), "\n")
    }
    
    if (!is.null(info$missing_cols) && length(info$missing_cols) > 0) {
      output_text <- paste0(output_text, "Missing Columns: ", paste(info$missing_cols, collapse = ", "), "\n")
    }
    
    # Pre-filter analysis
    if (!is.null(info$is_weekend_type)) {
      output_text <- paste0(output_text, "\n--- Pre-Filter Analysis ---\n")
      output_text <- paste0(output_text, "Is_Weekend type: ", info$is_weekend_type, "\n")
      output_text <- paste0(output_text, "Is_Weekend unique values: ", paste(info$is_weekend_unique, collapse = ", "), "\n")
      if (!is.null(info$is_weekend_summary)) {
        output_text <- paste0(output_text, "Is_Weekend summary:\n")
        for (i in 1:length(info$is_weekend_summary)) {
          output_text <- paste0(output_text, "  ", names(info$is_weekend_summary)[i], ": ", info$is_weekend_summary[i], "\n")
        }
      }
      if (!is.null(info$is_weekend_converted)) {
        output_text <- paste0(output_text, "Is_Weekend converted: ", info$is_weekend_converted, "\n")
      }
      output_text <- paste0(output_text, "\nMonth type: ", info$month_type, "\n")
      output_text <- paste0(output_text, "Month unique values: ", paste(info$month_unique, collapse = ", "), "\n")
      if (!is.null(info$month_summary)) {
        output_text <- paste0(output_text, "Month summary:\n")
        for (i in 1:length(info$month_summary)) {
          output_text <- paste0(output_text, "  Month ", names(info$month_summary)[i], ": ", info$month_summary[i], "\n")
        }
      }
    }
    
    if (!is.null(info$rows_after_filter)) {
      output_text <- paste0(output_text, "\n--- After Filtering ---\n")
      output_text <- paste0(output_text, "Rows After Filtering: ", info$rows_after_filter, "\n")
      output_text <- paste0(output_text, "Unique Stations: ", info$unique_stations, "\n")
      output_text <- paste0(output_text, "Unique Dates: ", info$unique_dates, "\n")
      
      # Additional diagnostic info if filtering failed
      if (!is.null(info$weekday_count)) {
        output_text <- paste0(output_text, "\nDiagnostics:\n")
        output_text <- paste0(output_text, "  Weekday rows (!Is_Weekend): ", info$weekday_count, "\n")
        output_text <- paste0(output_text, "  Non-winter rows (!Month in 12,1,2): ", info$non_winter_count, "\n")
        output_text <- paste0(output_text, "  Weekday AND non-winter: ", info$weekday_non_winter_count, "\n")
      }
    }
    
    if (!is.null(info$rows_after_grouping)) {
      output_text <- paste0(output_text, "\nRows After Grouping: ", info$rows_after_grouping, "\n")
      output_text <- paste0(output_text, "Min n_days: ", info$min_n_days, "\n")
      output_text <- paste0(output_text, "Max n_days: ", info$max_n_days, "\n")
      output_text <- paste0(output_text, "Stations with >= 30 days: ", info$stations_with_30plus_days, "\n")
    }
    
    if (!is.null(info$rows_after_filter30)) {
      output_text <- paste0(output_text, "\nRows After Filtering (n_days >= 30): ", info$rows_after_filter30, "\n")
    }
    
    if (!is.null(info$final_rows)) {
      output_text <- paste0(output_text, "\nFinal Rows: ", info$final_rows, "\n")
    }
    ...
    if (!is.null(info$success) && info$success) {
